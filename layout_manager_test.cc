...
TEST_F(LayoutManagerTest, CycleTabs) {
  // Make sure that we don't crash if we don't have any windows.
  lm_->CycleSelectedTab(true);
  lm_->CycleSelectedTab(false);

  // Create a toplevel window that says it has a single tab.
  XWindow xid = CreateToplevelWindow(1, 0, 0, 0, 640, 480);
  SendInitialEventsForWindow(xid);
  MockXConnection::WindowInfo* info = xconn_->GetWindowInfoOrDie(xid);
  info->client_messages.clear();

  // Cycling should be a no-op in this case too.
  lm_->CycleSelectedTab(true);
  EXPECT_TRUE(info->client_messages.empty());

  // Tell the window manager that we have three tabs total and have
  // selected the second one.
  ChangeTabInfo(xid, 3, 1, wm_->GetCurrentTimeFromServer());
  SendWindowTypeEvent(xid);
  LayoutManager::ToplevelWindow* toplevel = lm_->GetToplevelWindowByXid(xid);
  ASSERT_EQ(3, toplevel->tab_count());
  ASSERT_EQ(1, toplevel->selected_tab());

  // Cycle forward to select the third tab.
  lm_->CycleSelectedTab(true);
  EXPECT_EQ(2, ConsumeTabSelectMessage(xid));
  ChangeTabInfo(xid, 3, 2, wm_->GetCurrentTimeFromServer());
  SendWindowTypeEvent(xid);

  // And forward again, which should make us wrap around to the first tab.
  lm_->CycleSelectedTab(true);
  EXPECT_EQ(0, ConsumeTabSelectMessage(xid));
  ChangeTabInfo(xid, 3, 0, wm_->GetCurrentTimeFromServer());
  SendWindowTypeEvent(xid);

  // If we cycle backward at this point, we should wrap to the third tab.
  lm_->CycleSelectedTab(false);
  EXPECT_EQ(2, ConsumeTabSelectMessage(xid));
  ChangeTabInfo(xid, 3, 2, wm_->GetCurrentTimeFromServer());
  SendWindowTypeEvent(xid);

  // Map a second toplevel window with two tabs, the second of which is
  // selected.
  XWindow xid2 = CreateToplevelWindow(2, 1, 0, 0, 640, 480);
  SendInitialEventsForWindow(xid2);
  MockXConnection::WindowInfo* info2 = xconn_->GetWindowInfoOrDie(xid2);
  info2->client_messages.clear();
  ASSERT_EQ(xid2, xconn_->focused_xid());

  // Cycle forward.  We should switch to the first toplevel and select its
  // first tab.
  lm_->CycleSelectedTab(true);
  EXPECT_EQ(xid, xconn_->focused_xid());
  EXPECT_EQ(0, ConsumeTabSelectMessage(xid));
  ...
  // If we cycle backward, we should switch back to the second toplevel.
  // Its last tab is already selected, so we shouldn't send a message to it.
  lm_->CycleSelectedTab(false);
  EXPECT_EQ(xid2, xconn_->focused_xid());
